...
var init = function(){
  ...
  // var gui = new dat.GUI();
  gui.add(cb, 'ptlGap').min(0).max(5).step(1).onChange(function(){
    cb.init(true);
  });
  gui.add(cb, 'ptlSize').min(1).max(5).step(1).onChange(function(){
    cb.init(true);
  });
  gui.add(cb, 'restless');
  ...
  (window.addEventListener
   ? window.addEventListener('click', function(){cb.init(true)}, false)
   : window.onclick = function(){cb.init(true)});
}
